...
import { PatternLibrary, commonPatterns } from './pattern-library'
import { RegexHistory } from './regex-history'
import { RegexExplainer } from './regex-explainer'
import { RegexChallenges } from './regex-challenges'
...
export default function RegexPlayground() {
  ...
  const handleFlagsChange = useCallback((checked: boolean, flag: string) => {
    dispatch({ type: 'SET_FLAGS', payload: checked ? state.flags + flag : state.flags.replace(flag, '') })
  ...
  const handleSavePattern = useCallback(() => {
    if (state.regex && state.patternName) {
      dispatch({
        ...
        payload: [...state.savedPatterns, { name: state.patternName, regex: state.regex, flags: state.flags }]
      })
      dispatch({ type: 'SET_PATTERN_NAME', payload: '' })
    }
  }, [state.regex, state.patternName, state.flags, state.savedPatterns, dispatch])

  const handleLoadPattern = useCallback((pattern: string) => {
    if (pattern === 'default') {
      dispatch({ type: 'SET_REGEX', payload: '' })
      dispatch({ type: 'SET_FLAGS', payload: 'g' })
    } else {
      const selectedPattern = state.savedPatterns.find(p => p.name === pattern)
      if (selectedPattern) {
        ...
        dispatch({ type: 'SET_REGEX', payload: commonPatterns[pattern].regex })
        dispatch({ type: 'SET_FLAGS', payload: 'g' })
      }
    }
  ...
  const handleDeletePattern = useCallback((patternName: string) => {
    dispatch({
      type: 'SET_SAVED_PATTERNS',
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className={`w-full max-w-4xl mx-auto ${isDarkMode ? 'dark' : ''}`}
      >
            ...
            <Tabs defaultValue="editor" className="space-y-4">
              ...
              <TabsContent value="library">
                <PatternLibrary
                  ...
                  onPatternNameChange={(e) => dispatch({ type: 'SET_PATTERN_NAME', payload: e.target.value })}
              ...
              <TabsContent value="history">
                <RegexHistory
                  ...
                  onSelectRegex={(regex) => dispatch({ type: 'SET_REGEX', payload: regex })}
                />
              </TabsContent>
